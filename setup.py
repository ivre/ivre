...
def collect_files(directory, extensions):
    ...
    Returns an empty list when the directory does not exist; this lets
    setup.py reference generated trees (e.g. ``web/static/ui/`` from
    ``pkg/buildwebui``) without crashing on a fresh checkout where the
...
setup(
    data_files=[
        ...
        # New web-ui bundle (generated by ``pkg/buildwebui``; never
        # edited by hand). The Vite build emits ``index.html`` at the
        # tree root and a single ``assets/`` subdirectory with hashed
        # JS/CSS plus optional source maps.
        (
            "share/ivre/web/static/ui",
            collect_files("web/static/ui", [".html", ".png", ".svg", ".ico"]),
        ),
        (
            "share/ivre/web/static/ui/assets",
            collect_files(
                "web/static/ui/assets", [".js", ".css", ".js.map", ".css.map"]
            ),
        ),
        # Bootstrap
        (
