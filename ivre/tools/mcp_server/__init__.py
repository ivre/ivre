...
def _register_tools() -> None:
    ...
    # --- Audit log (read-only) ---
    #
    # ``DBAudit`` is an append-only security log; MCP exposes the
    # read surface only.  ``record`` happens via the web hooks
    # (``ivre/web/audit_hook.py``); ``purge_older_than`` is an
    # operator-only verb on the CLI.  The three tools below
    # cover the read shapes the future UI needs.
    #
    # Auth gate: per-user by default, admin sees all -- mirrors
    # the ``/cgi/audit/*`` web routes.  An unauthenticated MCP
    # caller (stdio transport, or HTTP with ``WEB_AUTH_ENABLED =
    # False``) sees only their own scope, which for the
    # unauthenticated case means ``user_email = None`` -- i.e.
    # the anonymous-actor events.  That is intentional: the
    # audit log is a security control and stdio callers running
    # on the same box as the audit backend can already inspect
    # the raw collection if they want broader visibility.

    def _require_audit_backend() -> None:
        """Raise ``McpError`` when no audit backend is wired.

        Mirrors :func:`_require_notes_backend`.
        """
        if db.audit is None:
            raise McpError(
                ErrorData(
                    code=INTERNAL_ERROR,
                    message=(
                        "Audit backend not available. Set DB_AUDIT "
                        "(or DB) to a backend with audit support "
                        "(MongoDB, PostgreSQL, or DuckDB) to enable it."
                    ),
                )
            )

    def _resolve_audit_reader() -> tuple[str | None, bool]:
        """Resolve ``(user_email, is_admin)`` for an audit read.

        Returns ``(None, False)`` for unauthenticated stdio
        callers; the caller-side enforcement then constrains
        the query to the anonymous-actor scope
        (``user_email=None``).
        """
        # pylint: disable=import-outside-toplevel
        ...
        return user, is_admin_email(user)
    ...
    def audit_query(
        ...
        ``event_type`` narrows to one of ``upload`` /
        ``admin_action`` / ``oversize_query``.

        ``user_email`` narrows to one user's events.  A
        non-admin caller can only supply their own email
        (any other value raises ``INVALID_PARAMS``); the
        per-user gate is enforced even when ``user_email``
        is unset (the query is forced to the caller's own
        scope).  An admin caller may filter freely.

        ``since`` / ``until`` are Unix timestamps (UTC) for
        the ``created_at`` window: ``since`` is inclusive,
        ``until`` is exclusive.

        ``limit`` is capped at 1000 (tighter than the web
        tier's ``WEB_MAXRESULTS`` because MCP responses
        round-trip through an LLM context window).
        """
        import datetime as _dt  # pylint: disable=import-outside-toplevel

        _require_audit_backend()
        caller, is_admin = _resolve_audit_reader()
        if not is_admin:
            if user_email is not None and user_email != caller:
                raise McpError(
                    ErrorData(
                        code=INVALID_PARAMS,
                        message=(
                            "Non-admin callers can only read their own " "audit trail"
                        ),
                    )
                )
            user_email = caller
    ...
    def audit_count(
        ...
        Same filter semantics as :func:`audit_query`.  Same
        per-user gate (admins see all; non-admins are forced
        to their own scope).
        """
        import datetime as _dt  # pylint: disable=import-outside-toplevel
            ...
            if user_email is not None and user_email != caller:
                raise McpError(
                    ErrorData(
                        code=INVALID_PARAMS,
                        message=(
                            "Non-admin callers can only count their own " "audit trail"
                        ),
                    )
                )
            user_email = caller
    ...
    @mcp.tool()
    def audit_get(event_id: str) -> str:
        """Fetch a single audit event by ``event_id``.

        ``event_id`` accepts any UUID textual form
        :class:`uuid.UUID` accepts (32-char hex, 36-char
        with dashes, ``{...}`` braced, or ``urn:uuid:...``).
        Returns the JSON-encoded event or ``"null"`` when
        no event matches.

        Non-admin callers only see events whose actor is
        themselves; events outside that scope return
        ``"null"`` (same shape as a missing event, so the
        tool does not leak the existence of out-of-scope
        events).
        """
        _require_audit_backend()
        caller, is_admin = _resolve_audit_reader()
        ...
        if not is_admin:
            ...
            if actor.get("user_email") != caller:
                return json.dumps(None)
        return json.dumps(event, default=serialize)
