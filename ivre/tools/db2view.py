...
def main():
    ...
    if args.view_category:
        db.view.category = args.view_category
    # Filter by ip for passive
    if args.view_source == 'passive' and args.ips:
        flt = None

        def _updtflt_(flt, nflt):
            if not flt:
                return nflt
            return db.passive.flt_or(flt, nflt)
        for a in args.ips:
            if '-' in a:
                a = a.split('-', 1)
                if a[0].isdigit():
                ...
                if a[1].isdigit():
                    a[1] = int(a[1])
                flt = _updtflt_(flt, db.passive.searchrange(a[0], a[1]))
            elif '/' in a:
                flt = _updtflt_(flt, db.passive.searchnet(a))
            else:
                if a.isdigit():
                    a = db.passive.ip2internal(int(a))
                flt = _updtflt_(flt, db.passive.searchhost(a))
